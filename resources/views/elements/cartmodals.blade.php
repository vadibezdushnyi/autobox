        ...
        <div class="content">
  			...
  			<div class="modal__header-content">
  				<div class="modal-header">
  					<h3 class="title">{{ $_page->import_modal->text_21 }}</h3>
  	            ...
  	            <div class="modal__content__container">
  	            	<form class="form-import" id="cart-import-form">
  	            		...
  	            		<div class="controls-block">
  	            			<h5 class="controls-block__title">{{ $_page->import_modal->text_1 }}</h5>
  	            			<div class="checkbox-blocks">
  	            					...
  	            					<div class="checkbox-block__content">
  	            						<div class="formats-table-wrapper">
  		            						<table class="formats-table">
  		            							<tbody>
  		            								<tr>
  		            									<td>{{ $_page->import_modal->text_2 }}</td>
  		            									<td>{{ $_page->import_modal->text_3 }}</td>
  		            									<td>{{ $_page->import_modal->text_4 }}</td>
  		            								</tr>
  		            							</tbody>
  		            						</table>
  		            					</div>
  	            				...
  	            				<label class="checkbox-block">
                                    ...
                                    <td>{{ $_page->import_modal->text_3 }}</td>
  		            									<td>{{ $_page->import_modal->text_4 }}</td>
                                    <td>{{ $_page->import_modal->text_5 }}</td>
  		            								</tr>
  		            							</tbody>
  		            						</table>
  	            						</div>
  	            		...
  	            		<div class="form-tabs-2 js_tabs-scope" data-tabs="1">
  	            			<div class="form-tabs-2__nav">
  	            				<button type="button" class="btn btn-tab-icon js_tabs-trigger active" data-tabs="1" data-tab="1">
  	            					<span class="btn-content icon icon-excel">{{ $_page->import_modal->text_6 }}<span class="mobile-hidden"> {{ $_page->import_modal->text_7 }}</span></span>
  	            				</button>
  	            				<button type="button" class="btn btn-tab-icon js_tabs-trigger" data-tabs="1" data-tab="2">
  	            					<span class="btn-content icon icon-import">{{ $_page->import_modal->text_8 }}<span class="mobile-hidden"> {{ $_page->import_modal->text_9 }}</span></span>
  	            				</button>
  	            			</div>
  	            			<div class="form-tabs-2__container">
  	            				<div class="tab js_tabs-target active" data-tabs="1" data-tab="1">
  				            		<div class="controls-block">
  				            			<h5 class="controls-block__title">{{ $_page->import_modal->text_10 }}</h5>
  				            			<div class="checkbox-blocks checkbox-blocks--inline">
  				            				<label class="checkbox-block">
  				            					<input type="radio" name="columns-delimiter" value="1" checked>
  				            					<span class="rectangle"></span>
  				            					<div class="checkbox-block__content">
  				            						<div class="text">{{ $_page->import_modal->text_11 }}</div>
  				            					</div>
  				            				</label>
  				            				<label class="checkbox-block">
  				            					...
  				            					<span class="rectangle"></span>
  				            					<div class="checkbox-block__content">
  				            						<div class="text">{{ $_page->import_modal->text_12 }}</div>
  				            					</div>
  				            				</label>
  				            			</div>
  				            		</div>
                      ...
                      <div class="form-style form-style--dark">
  	                    <div class="form-footer">
  	                        <div class="form-message icon icon-attention message-error active" style="visibility: hidden;">
  	                            {{ $_page->import_modal->text_13 }}
  	                        </div>
  	                        <div class="form-message icon icon-success message-success active" style="visibility: hidden;">
  	                            {{ $_page->import_modal->text_14 }}
  	                        </div>
  											<div class="action-buttons active">
  												<button type="reset" class="btn btn-grey-small js_modal-close" onclick="__.importDiscard()">{{ $_page->import_modal->text_15 }}</button>
  												<button type="button" class="btn btn-red-small submit" onclick="__.cartImport(event)">{{ $_page->import_modal->text_16 }}</button>
  											</div>
  					                    </div>
  					                </div>
  	            				...
  	            				<div class="tab js_tabs-target" data-tabs="1" data-tab="2">
  				            		<div class="controls-block">
  				            			<h5 class="controls-block__title">{{ $_page->import_modal->text_17 }}</h5>
  				            			<div class="file-uploader js_file-uploader">
  				            				<label class="file-input">
  				            					<input type="file" id="import-xls-input">
  				            					<span class="btn file-input__btn icon icon-import">{{ $_page->import_modal->text_18 }}</span>
  				            				</label>
				                    ...
				                    <div class="form-style form-style--dark">
					                    <div class="form-footer">
				                        <div class="form-message icon icon-attention message-error active" style="visibility: hidden;">
				                            {{ $_page->import_modal->text_19 }}
				                        </div>
				                        <div class="form-message icon icon-success message-success active" style="visibility: hidden;">
				                            {{ $_page->import_modal->text_20 }}
				                        </div>
          											<div class="action-buttons active">
          												<button type="reset" class="btn btn-grey-small" onclick="__.importDiscard()">{{ $_page->import_modal->text_15 }}</button>
          												<button type="button" class="btn btn-red-small submit" onclick="__.cartImport(event)">{{ $_page->import_modal->text_16 }}</button>
          											</div>
					                    </div>
					                </div>
...
<div class="modal__window modal__window--general" data-modal-type="add-products-result-old">
    <div class="modal__wrapper">
        <div class="modal__container">
			<div class="modal__header-content">
				<div class="modal-header">
					<h3 class="title">{{ $_page->import_modal->text_22 }}</h3>
					<button class="btn-modal-close icon icon-cross3 js_modal-close"></button>
				</div>
			</div>
			...
			<div class="modal__content">
				<div class="modal__content__container">
					...
					<div class="controls-block">
						<h5 class="controls-block__title"><span id="import-matches-count">0</span> {{ $_page->import_modal->text_23 }}</h5>
						<span class="import-errors">{{ $_page->import_modal->text_24 }}: <span id="import-errors-count">0</span>  </span>
					</div>
          ...
          <div class="cart-table-container">
              <table class="table-style-black table-import-result">
                  <thead>
                      <tr>
                          <td class="cell-partcode">{{ $_page->import_modal->text_25 }}</td>
                          <td class="cell-quantity">{{ $_page->import_modal->text_26 }}</td>
                          <td class="cell-comment">{{ $_page->import_modal->text_27 }}</td>
                          <td class="cell-error">{{ $_page->import_modal->text_28 }}</td>
                      </tr>
                  </thead>
                  <tbody>
                      <tr>
                          <td class="cell-partcode" data-sort="10001">
                              <div class="title-mobile"><span class="text">{{ $_page->import_modal->text_25 }}</span></div>
                              <div class="cell-value"><span class="nowrap">-</span></div>
                          </td>
                          <td class="cell-quantity" data-sort="10">
                          	<div class="title-mobile"><span class="text">{{ $_page->import_modal->text_26 }}</span></div>
                              <div class="cell-value">-</div>
                          </td>
                          <td class="cell-comment" data-cell="cell-comment">
                              <div class="title-mobile"><span class="text">{{ $_page->import_modal->text_27 }}</span></div>
                          ...
                          <td class="cell-error">
                          	<div class="title-mobile"><span class="text">{{ $_page->import_modal->text_28 }}</span></div>
                              <div class="cell-value">-</div>
                          </td>
                      </tr>
                  </tbody>
          ...
          <div class="form-style form-style--dark">
            <div class="form-footer">
              <div class="action-buttons active">
              	<button class="btn btn-icon btn-black btn-add-products js_modal-open" data-modal-type="add-products"><span class="icon"><span class="plus"></span></span>{{ $_page->import_modal->text_32 }}</button>
              </div>
            </div>
        </div>
    ...
    <div class="modal__wrapper">
      <div class="modal__container">
        <div class="content">
    			...
    			<div class="modal__header-content">
    				<div class="modal-header">
    					<h3 class="title">{{ $_page->substitution_modal->text_1 }}</h3>
    					<button class="btn-modal-close icon icon-cross3 js_modal-close"></button>
    				</div>
    			</div>
          ...
          <div class="modal__content">
            <div class="modal__content__container">
              <div class="brand-info-container" style="padding-left:0;">
                  <div class="logo-image"><img alt="" src="{{ url('/public/img/icons-general/car-logos/Car-logos_Audi.svg') }}"></div>
                  <form class="product-overview">
                      <h3 class="partcode-title"><small>{{ $_page->substitution_modal->text_2 }}</small>194439005</h3>
                      <table class="product-short-info">
                          <tbody>
                            <tr>
                                <td>{{ $_page->substitution_modal->text_3 }}:</td>
                                <td><b> - </b></td>
                            </tr>
                            <tr>
                                <td>{{ $_page->substitution_modal->text_4 }}:</td>
                                <td><b> - </b></td>
                            </tr>
                            <tr>
                                <td>{{ $_page->substitution_modal->text_5 }}:</td>
                                <td><b> - </b></td>
                            </tr>
                            <tr>
                                <td>{{ $_page->substitution_modal->text_6 }}:</td>
                                <td><b> - </b></td>
                            </tr>
                            <tr>
                                <td>{{ $_page->substitution_modal->text_7 }}:</td>
                                <td><b> - </b></td>
                            </tr>
                          </tbody>
                      </table>
                      <div class="cart-footer-actions">
                          <div class="total-amount">
                              <span class="text">{{ $_page->substitution_modal->text_8 }}</span><span class="value">0,00</span>
                          </div>
                          <div class="action-buttons">
                              <button type="button" class="btn btn-red-small" onclick="__.cartReplacement()">{{ $_page->substitution_modal->text_9 }}</button>
                          </div>
                      </div>
                  </form>
              </div>
              <button type="button" class="btn btn-grey-small js_modal-close">{{ $_page->substitution_modal->text_10 }}</button>
            </div>
          </div>
        </div>
      </div>
