...
<title>Action LOAD BUFFER LAST DIALOG FILE</title>
</head>
...
<?php 
	...
	$query = "SELECT  * FROM [pre]dialog_files_ref WHERE `adminMod`='".$_POST['admin_id']."' AND `ref_table`='".$_POST['ref_table']."' AND `ref_id`='".$_POST['ref_id']."' ORDER BY id DESC LIMIT 1";
	...
	if($rf != null)
	{
		...
		?>
				<div class="item" id="rf-item-<?php echo $rf['id'] ?>">
                	<img class="close-file-item" alt="X" src="<?php echo WP_FOLDER ?>img/close-icon.png" onclick="delete_dialog_rf(<?php echo $rf['id'] ?>);">
