...
<?php 
	...
	foreach($list as $item_id)
	{
		...
		$update_query = "UPDATE [pre]stock_fields SET `block`=1 WHERE `id`='".$item_id."' LIMIT 1";

			$update_stmt = $dbh->prepare($update_query);
			$update_arr = $update_stmt->execute();
		
		?>
		<p>Складская ячейка <b><?php echo $data['stock_id'].".".$data['zona'].".".$data['rack'].".".$data['section'].".".$data['shelf'] ?></b> заблокирована.</p>
		<?php
	}
?>
	</div>
