...
<title>Dialog file upload</title>
</head>
...
<?php
	...
	$is_link 	= 0;//$_POST['is_link'][0];
	$is_target 	= 0;//$_POST['is_target'][0];
...
<?php 
	if(isset($_FILES['file']) && $_FILES['file']['size'] > 0 && $admin_id > 0)
	{
		...
		if($filename)
		{
			...
			$query = "INSERT INTO [pre]dialog_files_ref (`ref_table`,`ref_id`,`file`,`crop`,`title`,`is_link`,`href`,`target`,`path`,`adminMod`) 
